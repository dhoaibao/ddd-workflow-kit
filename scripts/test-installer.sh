...
assert [ "$(awk -F '\t' '$2 == "document" { n++ } END { print n + 0 }' "$ROOT/PACKAGES")" = "$(find "$HOME/.agents/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" ]
...
if command -v script >/dev/null 2>&1 && script -qec true /dev/null >/dev/null 2>&1; then
  ...
  # Toggle Shared, move to Codex and toggle it, confirm; choose project; then
  # confirm the optional-packages prompt with no implementation packages selected.
  ...
  assert [ "$(find "$PTY_PROJECT/.agents/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" = "$(awk -F '\t' '$2 == "document" { n++ } END { print n + 0 }' "$ROOT/PACKAGES")" ]
  assert_not_exists "$PTY_PROJECT/.agents/skills/ddd-impl-fastapi-hdx"
...
printf '9. package-class selection: defaults, --skill, update, deselect, and unknown packages\n'
DOC_COUNT=$(awk -F '\t' '$2 == "document" { n++ } END { print n + 0 }' "$ROOT/PACKAGES")
SEL_HOME="$TMP/selection-home"
mkdir -p "$SEL_HOME"
HOME="$SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --agent pi --global
assert [ "$(find "$SEL_HOME/.pi/agent/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" = "$DOC_COUNT" ]
assert_not_exists "$SEL_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx"
HOME="$SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --update --skill ddd-impl-fastapi-hdx
assert [ -L "$SEL_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx" ]
assert [ "$(find "$SEL_HOME/.pi/agent/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" = "$((DOC_COUNT + 1))" ]
assert [ "$(python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); print("ddd-impl-fastapi-hdx" in d.get("skills", "").split(","))' "$SEL_HOME/.ddd-workflow-kit/manifest.json")" = True ]
HOME="$SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --update
assert [ -L "$SEL_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx" ]
assert [ "$(find "$SEL_HOME/.pi/agent/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" = "$((DOC_COUNT + 1))" ]
HOME="$SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --update --skill ddd
assert_not_exists "$SEL_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx"
assert [ "$(find "$SEL_HOME/.pi/agent/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" = "$DOC_COUNT" ]
if HOME="$SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --update --skill not-a-real-package; then
  printf 'test-installer: unknown --skill package unexpectedly succeeded\n' >&2; exit 1
fi
assert [ "$(find "$SEL_HOME/.pi/agent/skills" -mindepth 1 -maxdepth 1 -type l | wc -l | tr -d ' ')" = "$DOC_COUNT" ]
SEL_DIST="$TMP/selection-dist"
bash "$ROOT/scripts/build-release.sh" "$SEL_DIST"
SEL_LISTING_TMP="$TMP/selection-listing"
mkdir -p "$SEL_LISTING_TMP"
tar -xzf "$SEL_DIST/ddd-workflow-kit.tar.gz" -C "$SEL_LISTING_TMP"
assert [ -f "$SEL_LISTING_TMP/PACKAGES" ]
SEL_ARCHIVE_HOME="$TMP/selection-archive-home"
mkdir -p "$SEL_ARCHIVE_HOME"
HOME="$SEL_ARCHIVE_HOME" bash "$ROOT/install.sh" --source-dir "$SEL_LISTING_TMP" --agent pi --global --skill all
assert [ -L "$SEL_ARCHIVE_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx" ]

printf '10. legacy manifest (no skills header) still removes a deleted package on update\n'
LEGACY_HOME="$TMP/legacy-skills-header-home"
mkdir -p "$LEGACY_HOME"
...
assert [ -L "$LEGACY_HOME/.pi/agent/skills/ddd" ]

printf '11. an unrelated second install preserves a previously opted-in optional package\n'
CROSS_SEL_HOME="$TMP/cross-selection-home"
mkdir -p "$CROSS_SEL_HOME"
HOME="$CROSS_SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --agent pi --global --skill ddd-impl-fastapi-hdx
assert [ -L "$CROSS_SEL_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx" ]
HOME="$CROSS_SEL_HOME" bash "$ROOT/install.sh" --source-dir "$ROOT" --agent claude --global
assert [ -L "$CROSS_SEL_HOME/.pi/agent/skills/ddd-impl-fastapi-hdx" ]
assert [ -L "$CROSS_SEL_HOME/.claude/skills/ddd-impl-fastapi-hdx" ]
