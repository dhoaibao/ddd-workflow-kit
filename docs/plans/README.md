...
A follow-on extension (recorded in the same [implementation-package plan](complete/ddd-impl-fastapi-hdx.md)) closes the approval-gated greenfield bootstrap path the MVP named but left unfulfilled: `references/bootstrap.md` and `assets/service-skeleton.md` name the residue decisions `hdx-domain-kit` leaves to a consumer's composition root (settings layer, authorizer wiring including the unstated `AllowAll()` default, publisher choice, consumer migration ordering, test tiers, project layout/tooling), and `SKILL.md` gained a named ordered bootstrap procedure with an explicit approval stop before any file is created. No new package, `PACKAGES` row, or installer/validator-class change was needed.
